...
def _cluster_into_bins(eval_data, ref_data, netD, num_clusters):
    ...
    for d in list(cluster_data):
        ...
        # logging.info(f"Discriminator's output: {l}. It is classified as: {classified}")
        cluster_data.remove(d)
        # labels.append(l)
        labels.append(classified)

    logging.info(f"length labels: {len(labels)}, length eval_data: {len(eval_data)}")
...
def compute_prd_from_embedding(eval_data, ref_data, netD, num_clusters=20,
