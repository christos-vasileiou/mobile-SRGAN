...
def print_time(ss):
    ...
    logging.info(f"Training's duration {hh}:{mm}:{ss}")
...
def run_a_sample(netG2, dataset, hps, training_typo, n_samples=1):
    ...
    if not netG2:
        ...
        try:
            if exists(f'./trained_models/{FLAGS.gentype}/{FLAGS.dataset}/Generator' + training_typo + '-final.pt'):
                state_dict = torch.load(f'./trained_models/{FLAGS.gentype}/{FLAGS.dataset}/Generator' + training_typo + '-final.pt', map_location=device)
                netG1.load_state_dict(state_dict)
                netG1 = netG1.to(device)
        except:
            logging.debug(f"netG1 couldn't load its trained weights!")
        try:
            if exists(f'./trained_models/{FLAGS.gentype}/{FLAGS.dataset}/Generator' + training_typo + '.pt'):
                state_dict = torch.load(f'./trained_models/{FLAGS.gentype}/{FLAGS.dataset}/Generator' + training_typo + '.pt', map_location=device)
                netG2.load_state_dict(state_dict)
                netG2 = netG2.to(device)
        except:
            logging.debug(f"netG2 couldn't load its trained weights!")
...
def get_trained_type(netG):
    ...
    print("training type: ", training_typo)
