...
def PSNR(y_true, y_pred, MAXp=1.):
    ...
    :param MAXp: maximum value of the pixel range (default=1.).
    :return: psnr metric
    """
    if not isinstance(MAXp, torch.Tensor):
        MAXp = torch.tensor(MAXp)
    if len(y_true.shape) != 4 or len(y_pred.shape) != 4:
        raise ValueError("Check PSNR")
    psnrs = []
    for p, t in zip(y_pred, y_true):
        # TODO: find what should be the MAXp and if Normalization is needed !
        p = normalize(p)
        t = normalize(t)
        psnr = 20*torch.log10(MAXp) - 10*torch.log10(torch.mean(torch.square(t-p)))
        psnrs.append(psnr)
