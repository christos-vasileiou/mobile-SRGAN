...
def make_encoder_layers_from_hps(hps, retain_shape_to_output=True, depthwise=False):
    ...
    for i in reversed(range(hps['depth'])):
        encoder = UnetBottleNeck(in_channels=int(hps['nchan'] * 2 ** (i - 1)),
                                 ...
                                 depthwise=depthwise,
                                 name="encoder_" + str(i))
        down = DepthWiseConvBn2d(int(hps['nchan'] * 2 ** i), int(hps['nchan'] * 2 ** i), kernel_size=2, stride=2, padding=0) if depthwise else ConvBn2d(int(hps['nchan'] * 2 ** i), int(hps['nchan'] * 2 ** i), kernel_size=4, stride=2, padding=1)
    ...
    def get_embedding(self, x):
        ...
        while hasattr(self, layer_name(i)):
            ...
            if i < 2 * self.hps['depth'] and i % 2 == 0:
                ...
                paste = cropped.pop(-1)
                paste = torch.cat([paste[:x.shape[0], :x.shape[1], :x.shape[2], :x.shape[3]], x], axis=1)
                x = layer(paste)
            else:
                x = layer(x)
            i += 1
