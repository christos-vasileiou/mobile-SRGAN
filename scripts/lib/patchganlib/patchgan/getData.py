...
from patchgan.utils import plot, bcolors
import logging
...
def getData(train_pct = 0.7, valid_pct = 0.15, use_real=False):
    ...
    if use_real:
        ...
        plot(X[0], name='./pics/SAR-data.pdf', title='SAR Data', equal=True)
        plot(Y[0], name='./pics/Ideal-data.pdf', title='Ideal Data', equal=True)
