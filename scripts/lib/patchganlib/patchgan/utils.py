...
def plot(data, name='test.pdf', title='', equal=False, aspect3d = (1,1,1), vmax=None):
    if vmax is None:
        vmax = data.max()
    with PdfPages(name) as pdf:
        ...
        im = ax1.contour((data), cmap=parula_map, vmax=data.max())
        ...
        ax2.plot_surface(x, y, d.T, cmap=parula_map, vmax=data.max(), rstride=1, cstride=1, linewidth=0., antialiased=False)
        ...
        ax3.view_init(30, 45)
        x, y = np.mgrid[:data.shape[0], :data.shape[1]]
        ax3.plot_surface(x, y, d.T, cmap=parula_map, vmax=data.max(), rstride=1, cstride=1, linewidth=0., antialiased=False)
        ...
        ax4.view_init(90, 90)
        x, y = np.mgrid[:data.shape[0], :data.shape[1]]
        ax4.plot_surface(x, y, d.T, cmap=parula_map, vmax=data.max(), rstride=1, cstride=1, linewidth=0., antialiased=False)
    ...
    ax5.view_init(90, 90)
    ax5.plot_surface(x, y, d.T, cmap=parula_map, vmax=data.max(), rstride=1, cstride=1, linewidth=0., antialiased=False)
    ax5.set_title('Heatmap 90°-angle View - ')
    ax5.set_xlabel('x-axis')
    ax5.set_ylabel('y-axis')
    fig.tight_layout()
    pyplot.axis('off')
    pyplot.savefig(name[-4]+'.png')
    pyplot.close()
...
def run_a_sample(netG2, dataset, hps, training_typo, n_samples=1):
    ...
    if not netG2:
        ...
        netG1 = load_model(netG1, f'./trained_models/{FLAGS.gentype}/{FLAGS.dataset}/patch{str(FLAGS.doutput_size)}/Generator{training_typo}-final.pt')
        netG2 = load_model(netG2, f'./trained_models/{FLAGS.gentype}/{FLAGS.dataset}/patch{str(FLAGS.doutput_size)}/Generator{training_typo}.pt')
        logging.info(f'./trained_models/{FLAGS.gentype}/{FLAGS.dataset}/patch{str(FLAGS.doutput_size)}/Generator' + training_typo + '.pt')
    ...
    for i in range(n_samples):
        ...
        try:
            plot(output1[0][0],
                 name=f"./trained_models/{FLAGS.gentype}/{FLAGS.dataset}/patch{str(FLAGS.doutput_size)}/{i}-gan_output1" + training_typo + '-depth' + str(FLAGS.depth) + '_nchan' + str(
                     FLAGS.nchan) + '.pdf', title='Denoised Data', vmax=ideal_image[0].max())
        except:
            logging.debug(f"{bcolors.FAIL}netG1's plot face an issue{bcolors.ENDC}")

        plot(output2[0][0],
                 name=f"./trained_models/{FLAGS.gentype}/{FLAGS.dataset}/patch{str(FLAGS.doutput_size)}/{i}-gan_output2" + training_typo + '-depth' + str(FLAGS.depth) + '_nchan' + str(
                     FLAGS.nchan) + '.pdf', title='Denoised Data', vmax=ideal_image[0].max())

        # extract contours of inputs and outputs.
        plot(sar_image.cpu().detach().numpy()[0], name=f"./trained_models/{FLAGS.gentype}/{FLAGS.dataset}/patch{str(FLAGS.doutput_size)}/{i}-input" + training_typo + '.pdf', title='Radar Data')
        if dataset.shape[1] == 2: # Only Synthetic Dataset contains Ideal
            plot(ideal_image[0], name=f'./trained_models/{FLAGS.gentype}/{FLAGS.dataset}/patch{str(FLAGS.doutput_size)}/{i}-ideal' + training_typo + '.pdf', title='Ideal Data')
