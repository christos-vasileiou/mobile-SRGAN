...
from data3d.gan_models import Generator3d, GeneratorDepthWise3d, GeneratorPipeline3d, DiscriminatorDepthWise3d, Discriminator3d, DiscriminatorDepthWise3dMBD
...
def plot_3d(data, filename):
    ...
    isomin = data.min() + isorange * 0.1
    isomax = data.min() + isorange * 0.9
...
def run_a_3d_sample(netG2, features, labels, hps, training_typo, device, n_samples=1):
    ...
    if not netG2:
        ...
        netG1 = load_model(netG1,f'./trained_models/Generator3d{training_typo}-final.pt')
        netG2 = load_model(netG2,f'./trained_models/Generator3d{training_typo}.pt')
        logging.info(f'./trained_models/{FLAGS.gentype}/patch{str(FLAGS.doutput_size)}/Generator' + training_typo + '.pt')
    ...
    for i in range(n_samples):
        ...
        sar_image = np.expand_dims(np.expand_dims(dataset[i], axis=0), axis=0).astype('float64')
        label = np.expand_dims(np.expand_dims(labels[i], axis=0), axis=0).astype('float64')
        logging.info(f"Sar Image: {sar_image.shape}, Label: {label.shape}")
        ...
        try:
            plot_3d(output1.cpu().detach().numpy()[0][0], filename=f"./trained_models/{i}-output3d-1{training_typo}_nchan{str(FLAGS.nchan)}.html")
        except:
            logging.debug(f"Error: netG1's plot face an issue")
        plot_3d(output2.cpu().detach().numpy()[0][0], filename=f"./trained_models/{i}-output3d-2{training_typo}_nchan{str(FLAGS.nchan)}.html")
        plot_3d(sar_image.cpu().numpy()[0][0], filename=f"./trained_models/{i}-input{training_typo}.html")
        plot_3d(label.cpu().numpy()[0][0], filename=f"./trained_models/{i}-ideal{training_typo}.html")
