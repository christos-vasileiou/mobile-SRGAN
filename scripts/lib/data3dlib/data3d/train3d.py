...
import time
from data3d.get3data import *
from data3d.Unet import *
from data3d.gan_models import *
from data3d.utils import *
from data3d.dataset import FeedDataset3d
import warnings
import os
import logging
from os.path import exists
from data3d.utils import bcolors
...
def train(netG, netD, n_epochs=100, batch_size=16, lr=0.001, schedFactor=0.1, schedPatience=10,
          weight_decay=0.14, beta1=0.5, workers=1, ngpu=1, device=None, pixel_lambda=100, perc_lambda=1.):
    ...
    for epoch in range(n_epochs):
        dataset = FeedDataset3d(mode=Mode.training, split=0.8, dataset_size=FLAGS.dataset_size)
        ...
        for i, data in enumerate(trainloader):
            ...
            if criterion_perc:
                ...
                k = 0
                lambda_var = [perc_lambda for _ in range(len(real_outputs)-2)]
            ...
            if g.max() > ideal.max()//2:
                torch.save(netG.state_dict(), './trained_models/Generator3d' + str(training_typo) + '.pt')
                torch.save(netD.state_dict(), './trained_models/Discriminator3d' + str(training_typo) + '.pt')
        ...
        ################################################
        evalset = FeedDataset3d(mode=Mode.testing, split=0.8, dataset_size=FLAGS.dataset_size)
    ...
    plt.ylabel("Loss")
    plt.legend()
    plt.savefig('./trained_models/GDLoss' + str(training_typo) + '.png')
    # Save the model checkpoint
    torch.save(netG.state_dict(), './trained_models/Generator3d' + str(training_typo) + '-final.pt')
    torch.save(netD.state_dict(), './trained_models/Discriminator3d' + str(training_typo) + '-final.pt')
...
def main(argv):
    ...
    _, netG, netD, training_typo = train(netG,
                    ...
                    ngpu = hps['ngpu'],
                    device = device,
                    perc_lambda=hps['perc_lambda'],
                    pixel_lambda = hps['pixel_lambda'])

    features, labels = get3Data()
    run_a_3d_sample(None, features, labels, hps, training_typo, device)
