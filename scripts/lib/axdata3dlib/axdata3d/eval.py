...
def _cluster_into_bins(eval_data, ref_data, netD, num_clusters):
    ...
    """
    cluster_data = torch.tensor(np.vstack([eval_data, ref_data]))
    cluster_data = list(cluster_data)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    labels = []
    netD = netD.to(device)
    for d in list(cluster_data):
        l = netD(d.to(device))[-1]
        l = l.cpu().detach().numpy()
        classified = 0. if l <.5 else 1.
        #print(f"Discriminator's output: {l}. It is classified as: {classified}")
        cluster_data.remove(d)
        #labels.append(l)
        labels.append(classified)

    #print(f"length labels: {len(labels)}, length eval_data: {len(eval_data)}")
...
def compute_prd_from_embedding(eval_data, ref_data, netD, num_clusters=2,
    ...
    if torch.is_tensor(eval_data[0]):
        eval_data = [t.numpy() for t in eval_data]
    if torch.is_tensor(ref_data[0]):
        ref_data = [t.numpy() for t in ref_data]

    #print(len(eval_data), type(eval_data), type(eval_data[0]))
    eval_data = np.array(eval_data, dtype=np.float64)
    ref_data = np.array(ref_data, dtype=np.float64)
    precisions = []
    recalls = []
    for _ in range(num_runs):
        eval_dist, ref_dist = _cluster_into_bins(eval_data, ref_data, netD, num_clusters)
...
def PRDplot(precision_recall_pairs, labels=None, out_path=None, legend_loc='lower left', dpi=300):
