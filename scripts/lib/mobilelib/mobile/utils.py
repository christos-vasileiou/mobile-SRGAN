...
def PSNR(y_true, y_pred, MAXp=1.):
    ...
    return 20*torch.log10(MAXp) - 10*torch.log10(torch.mean(torch.square(y_true-y_pred)))
...
def build_models(hps, gentype="depthwise"):
    ...
    netG=None
    netD=None
    if gentype == 'depthwise':
        netG = GeneratorDepthWise(hps, retain_shape_to_output=True)
    elif gentype == 'pipeline':
        netG = GeneratorPipeline(hps)
    elif gentype == 'normal':
        netG = Generator(hps, retain_shape_to_output=True)
    return netG, netD
...
def run_a_sample(netG2, dataset, hps, training_typo, gentype="depthwise", n_samples=1):
    dataset = np.array(dataset)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    if not netG2:
        netG2, _ = build_models(hps)
        netG2 = load_model(netG2, f'./trained_models/{gentype}/Generator{training_typo}.pt')
    ...
    for i in range(n_samples):
        ...
        if dataset.shape[1] == 2: # pairs => synthetic dataset
            sar_image = np.expand_dims(dataset[i][0], axis=0).astype('float32')
            ideal_image = np.expand_dims(dataset[i][1], axis=0).astype('float32')
        elif dataset.shape[1] == 1: # 1 sample => real's dataset
            sar_image = np.expand_dims(dataset[i], axis=0).astype('float32')
        ...
        with torch.no_grad():
            ...
            startTime = time.time()
            output2 = netG2(sar_image)
            endTime = time.time()
            output2 = output2.cpu().detach().numpy()
            print(f"output2: min={output2.min()}, max={output2.max()}")
            print(f"Inference in {endTime - startTime} seconds\n\n")

        plot(output2[0][0],
                 name=f"./trained_models/{gentype}/{i}-gan_output2" + training_typo + '-depth' + str(hps['depth']) + '_nchan' + str(
                     hps['nchan']) + '.pdf', title='Denoised Data')

        # extract contours of inputs and outputs.
        plot(sar_image.cpu().detach().numpy()[0], name=f"./trained_models/{gentype}/{i}-input" + training_typo + '.pdf', title='Radar Data')
        if dataset.shape[1] == 2: # Only Synthetic Dataset contains Ideal
            plot(ideal_image[0], name=f'./trained_models/{gentype}/{i}-ideal' + training_typo + '.pdf', title='Ideal Data')
...
def run_a_real_sample(netG2, dataset, hps, training_typo, device, n_samples=1):
    ...
    # --- m -> sample of a dataset
    for i in range(n_samples):
        print(f"dataset: {dataset.shape}, input: {dataset[i].shape}")
