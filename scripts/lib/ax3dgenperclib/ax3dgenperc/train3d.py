...
import time
from ax3dgenperc.get3data import *
from ax3dgenperc.Unet import *
from ax3dgenperc.gan_models import *
from ax3dgenperc.utils import *
from ax3dgenperc.dataset import FeedDataset3d
import warnings
import os
import logging
...
def train(netG, netD, hps, ax_trial=None):
    ...
    for epoch in range(n_epochs):
        ...
        for i, data in enumerate(trainloader):
            ...
            errG_perc = torch.tensor(0.).to(device)
            ...
            if criterion_perc:
                ...
                k = 0
                lambda_var = [perc_lambda[i] for i in
                              range(len(real_outputs) - 2)]  # len(real_outputs) = layers of Discriminator = 5
                for ro, fo, lv in zip(real_outputs, fake_outputs, lambda_var):
                    ...
                    errG_perc += criterion_perc(ro, fo) * lv
                fake_outputs = [f.detach() for f in fake_outputs]
                # Calculate the gradients for this batch, accumulated (summed) with previous gradients
                errG_perc.backward()
                errG_perc = errG_perc.detach()
                errG += errG_perc
            ...
            if i % 100 == 0 or i == iterations - 1:
                ...
                logging.info(f"{epochTimes[-1]:.2f}sec: [{epoch}/{n_epochs}][{i}/{len(trainloader)}] Loss_D: {errD.item():.2f} Loss_D_perc: {errG_perc.item():.2f} Loss_G: {errG.item():.2f} Loss_G_p2p: {errG_p2p.item():.2f} G_max: {g.max().item():.3f} PSNR: {psnr:.2f} RMSE: {rmse:.2f} D(x): {D_x:.2f} D(G(z)): {D_G_z1:.2f}/{D_G_z2:.2f}")
                startTime = time.time()
        ...
        del errD, errG_perc # , g, z, psnr, rmse, real_outputs, fake_outputs,  errG, errG_p2p, dataset, trainloader
...
def main(argv):
    ...
    wandb.init(project="Testing", entity="chrivasileiou", config=hps)
