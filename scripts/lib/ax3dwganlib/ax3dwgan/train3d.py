...
import time
from ax3dwgan.get3data import *
from ax3dwgan.Unet import *
from ax3dwgan.gan_models import *
from ax3dwgan.utils import *
from ax3dwgan.dataset import FeedDataset3d
import warnings
import os
import logging
...
def train(netG, netD, hps, ax_trial=None):
    ...
    # Specify how many more times Critic (Discriminator) will be trained than Generator
    critic_iter = 5
    LAMBDA_TERM = 10.
    ...
    for epoch in range(n_epochs):
        ...
        for i, data in enumerate(trainloader):
            ...
            netD = unfreeze(netD)
            for i, d_iter in enumerate(range(critic_iter)):
                ...
                output = real_outputs[-1].view(-1).to(device)
                ...
                netD = netD.cpu()
                netG = netG.cuda()
                fake = netG(inputs).detach().to(device)
                ...
                gradient_penalty = calculate_gradient_penalty(ideal, fake, netD) * LAMBDA_TERM
                # Calculate Wasserstein + Gradient Penalty
                errD = d_loss_fake - d_loss_real + gradient_penalty
                Wasserstein_loss = d_loss_fake - d_loss_real
                print(i)
                errD.backward(retain_graph=True) #if i!=critic_iter-1 else False
                optimizerD.step()

            netD.zero_grad()
            # TODO: Fix perceptual backward. Doesn't work. Opt out perc loss!
            ...
            if criterion_perc:
                ...
                errD_perc.backward(create_graph=True, retain_graph=True)
            ...
            #           f'Train D Loss Total (fake - real{gp})': errD.item(),
...
def main(argv):
    ...
    #wandb.init(project="tiftrc-3d-WGAN", entity="chrivasileiou", config=hps)
