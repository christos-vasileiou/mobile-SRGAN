...
from ax3dwgan.gan_models import Generator3d, GeneratorDepthWise3d, GeneratorPipeline3d, DiscriminatorDepthWise3d, Discriminator3d, DiscriminatorDepthWise3dMBD
...
def calculate_gradient_penalty(real, fake, netD):
    ...
    eta = torch.FloatTensor(real.size(0), 1, 1, 1, 1).uniform_(0, 1).repeat(1, real.size(1), real.size(2),  real.size(3), real.size(4))
    eta = eta.to(device)
...
def hps_initialization():
    ...
    hps = {
        ...
        'n_epochs': n_epochs,
        'learning_rateG': learning_rate,
        'learning_rateD': learning_rate,
        'schedFactor': schedFactor,
        'schedPatience': schedPatience,
        'weight_decayG': weight_decay,
        'weight_decayD': weight_decay,
        'perc_lambda_1': perc_lambda,
        'perc_lambda_2': perc_lambda,
        'perc_lambda_3': perc_lambda,
        'perc_lambda_4': perc_lambda,
        'perc_lambda_5': perc_lambda,
...
def run_a_3d_sample(netG2, features, labels, hps, training_typo, device, n_samples=1, ax_trial=None):
    if features is None:
        features = []
        labels = []
