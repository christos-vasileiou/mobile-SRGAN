...
def make_encoder_layers_from_hps(hps):
    encoder_layers = []
    i = 0
    while i in range(hps['n_conv_layers']):
        ...
        stride = hps[f"strides_{i}"]
        kernel_size = hps[f"kernel_sizes_{i}"]
        padding = hps[f"paddings_{i}"]
        if i < hps['n_conv_layers']-1:
            ...
            #encoder_layers.append(Vectorize())
        i = i + 1
...
class DenoiseNet_ax(nn.Module):
    """Neural Network module.
        ...
        --- name:
    """
    def __init__(self, hps, printForward = False):
        ...
        self.encoder_layers = make_encoder_layers_from_hps(hps)
        #self.decoder_layers = self.__create_decoder()
    ...
    def get_embedding(self, x):
        ...
        while hasattr(self, layer_name(i)):
            ...
            x = layer(x)
            #if i < self.hps['n_conv_layers'] and self.printShape:
